...
frontier *initialize_frontier(char *frontier_folder) {
    frontier *frntr = (frontier *)malloc(sizeof(frontier));
...
frontier_item *frontier_add(frontier *frntr, uint8_t depth, uint64_t hash,
                            char *url) {
    frontier_item *item = (frontier_item *)malloc(sizeof(frontier_item));
    if (item == NULL)
        return NULL;
...
frontier_item *_frontier_read_one(frontier *frntr) {
    ...
    item->url = (char *)malloc(item->url_len + 1);
...
frontier_item *_copy_frontier_item(frontier_item *item) {
    frontier_item *copy = (frontier_item *)malloc(sizeof(frontier_item));
    copy->depth = item->depth;
    copy->url_len = item->url_len;
    copy->url_hash = item->url_hash;
    copy->url = (char *)malloc(sizeof(char) * item->url_len);
...
frontier_item *frontier_pop(frontier *frntr) {
    ...
    fflush(frntr->frontier_offset);
    fsync(fileno(frntr->frontier_offset));
